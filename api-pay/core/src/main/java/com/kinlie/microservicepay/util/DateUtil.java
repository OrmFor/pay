...
import com.kinlie.microservicepay.constant.ConstantDateType;
...
public class DateUtil {
	...
	/**
	* @Author wwy
	* @Description mongodb 存在bug 当入库时会减少8小时，入库之前增加8小时
	**/
	...
	public static Date getMongoDate1() {
		Date d = new Date();
	...
	//根据选择日期类型返回日期时间
	public static Map<String,Date> getDateByType(String type,Date startDay,Date endDay){

		switch (type){
			case ConstantDateType.TODAY://今天
				endDay = new Date();
				startDay = DateUtil.getFirstSecIntegralTime(endDay);
				break;
			case ConstantDateType.YESTERDAY://昨日
				startDay = DateUtil.getFirstSecIntegralTime(DateUtil.rollDay(new Date(), -1));
				endDay = DateUtil.getLastSecIntegralTime(DateUtil.rollDay(new Date(), -1));
				break;
			case ConstantDateType.WEEK://本周
				startDay = DateUtil.getWeekStartDate();
				endDay = new Date();
				break;
			case ConstantDateType.MONTH://本月
				startDay = DateUtil.getFirstDayOfMonth();
				endDay = new Date();
				break;
			case ConstantDateType.THREE_MONTH:
				endDay = new Date();
				startDay = DateUtil.rollDay(endDay , -90);
				break;
			case ConstantDateType.SELECT_DAY://筛选
				break;
			default:
				break;
		}
